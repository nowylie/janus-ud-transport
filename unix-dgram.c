...
void *recv_thread(void *data) {
   ...
   int stop = 0;
	...
	JANUS_LOG(LOG_INFO, "%s unix thread started\n", (admin ? "Admin" : "Janus"));
	while (!stop) {
		pthread_mutex_lock(&thread_lock);
		stop = thread_stop;
		pthread_mutex_unlock(&thread_lock);
		...
		if (n == -1) { // an error occured
			JANUS_LOG(LOG_ERR, "recvfrom: %s\n", strerror(errno));
			break;
		}
		b.data[n] = '\0';
		b.length = n;
		
		JANUS_LOG(LOG_HUGE, "received: %s\n", b.data);
		...
		JANUS_LOG(LOG_HUGE, "remote address: %s\n", uaddr->sun_path);
	...
	pthread_mutex_unlock(&thread_lock);
	JANUS_LOG(LOG_INFO, "%s thread stopped\n", (admin ? "admin" : "janus"));
...
int send_message(struct sockaddr_un *addr, gboolean admin, json_t *msg) {
	...
	if (json_dump_callback(msg, (json_dump_callback_t)dump_callback, &b, JSON_COMPACT)) {
		JANUS_LOG(LOG_ERR, "json_dump_callback: encoding too long for buffer\n");
		return -1;
	}
	b.data[b.length] = '\0';

	JANUS_LOG(LOG_HUGE, "sending: %s\n", b.data);
	JANUS_LOG(LOG_HUGE, "remote address: %s\n", addr->sun_path);

	socklen_t addrlen = UN_LEN(addr);
	ssize_t n = sendto(fd, b.data, b.length, 0, (struct sockaddr *)addr, addrlen);
	if (n == -1) {
		JANUS_LOG(LOG_ERR, "sendto: %s\n", strerror(errno));
		return -1;
	}
...
int janus_ud_init(janus_transport_callbacks *callbacks, const char *cpath) {
	...
	if (config == NULL) {
		JANUS_LOG(LOG_FATAL, "Unable to parse configuration file\n");
		return -1;
	}
	...
	if (item && item->value) {
		...
		if (path[PMAX - 1] != '\0') {
			JANUS_LOG(LOG_FATAL, "Admin socket path is too long");
			return -1;
		}
		...
		if (adminfd == -1) {
			JANUS_LOG(LOG_ERR, "new_unix_socket: %s\n", strerror(err));
		}
	...
	if (item && item->value) {
		...
		strncpy(path, item->value, PMAX);
		if (path[PMAX - 1] != '\0') {
			JANUS_LOG(LOG_FATAL, "Janus socket path is too long");
			return -1;
		}
